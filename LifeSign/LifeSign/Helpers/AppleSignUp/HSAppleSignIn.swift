...
//  LifeSign
//
//  Created by Haider Ali on 10/29/20.
...
//    static let shared = HSAppleSignIn()
    ...
    @available(iOS 13.0, *)
    func loginWithApple(appleSignupButton: DesignableButton, completionBlock:AppleSignInBlock){
        
        /* let appleButton = ASAuthorizationAppleIDButton.init(type: type!, style:style!)
        appleButton.translatesAutoresizingMaskIntoConstraints = false
        appleButton.layer.cornerRadius = 10.0 */
        
        // appleButton.addTarget(self, action: #selector(self.didTapLoginWithApple), for: .touchUpInside)
        
        appleSignupButton.addTarget(self, action: #selector(self.didTapLoginWithApple), for: .touchUpInside)
